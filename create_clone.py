...
def createast(dataaset, datalist):
    asts = []
    paths = []
    alltokens = []
    if dataaset == "GCJ":
...
def creategmndata(dataset, id, treedict, vocablen, vocabdict, device):
    trainfile = None
    validfile = None
    testfile = None
    if dataset == "GCJ":
    ...
    testdata = []
    print("train data")
    traindata = createpairdata(dataset, treedict, trainlist, device=device)
    print("valid data")
    validdata = createpairdata(dataset, treedict, validlist, device=device)
    print("test data")
    testdata = createpairdata(dataset, treedict, testlist, device=device)
    return traindata, validdata, testdata


def get_datalist(dataset, id):
    if dataset == "GCJ":
        indexdir = "javadata/"
        if id == "0":
            trainfile = open(indexdir + "trainall.txt")
    ...
    testlist = testfile.readlines()
    lists = []
    lists.extend(trainlist)
    lists.extend(validlist)
    lists.extend(testlist)
    path_list = []
    for line in lists:
        pairinfo = line.split()
        if dataset == "BCB":
        ...
        path_list.append(code1path)
        path_list.append(code2path)
    return list(set(path_list))


def createpairdata(dataset, treedict, pathlist, device):
    datalist = []
    countlines = 1
