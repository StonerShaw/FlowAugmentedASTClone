...
class GMNlayer(MessagePassing):
    ...
    def propagate_match(self, edge_index, size=None, **kwargs):
        ...
        out_attn = scatter_add(out_attn, edge_index[i], dim=0, dim_size=size[i])
        # print(out_attn.size())
    ...
    def forward(
        ...
        # u1 = torch.bmm(attn_1.reshape(n1, 1, n2), diff).reshape(n1, -1)
        # u2 = torch.bmm(attn_2.reshape(n2, 1, n1), diff.transpose(0, 1)).reshape(n2, -1)
        
        # print(u1.shape, u2.shape, flush=True)
        # print(m1.shape, m2.shape, flush=True)
...
class GMNnet(torch.nn.Module):
    def __init__(self, vocablen, embedding_dim, num_layers, device):
        ...
        self.embed = nn.Embedding(vocablen, embedding_dim, device=self.device)
        self.edge_embed = nn.Embedding(20, embedding_dim, device=self.device)
        # self.gmn=nn.ModuleList([GMNlayer(embedding_dim,embedding_dim) for i in range(num_layers)])
        self.gmnlayer = GMNlayer(embedding_dim, embedding_dim, device=self.device)
